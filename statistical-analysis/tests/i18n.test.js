...
test('every declared content and accessible-name key has Chinese and English values', () => {
...
test('formula sheet and hypothesis controls contain no legacy unmarked teaching copy', () => {
  const legacyFragments = [
    '<h3>1. Central Tendency & Dispersion</h3>',
    '<span class="cheat-label">Sample Mean</span>',
    '<h3>⚡ Key Rules & Concepts</h3>',
    '<span>F-Test (ANOVA / Variances)</span>',
  ...
  assert.match(html, /data-i18n="cheat_pvalue_desc"/);
  assert.match(html, /data-i18n="test_tab_f"/);
});

test('startup localization preserves structured CLT IPA content and refreshes dynamic text', () => {
  assert.match(html, /<h1 data-i18n="hero_title">Understand Statistics Through Data, Change, and Inference<\/h1>/);
  assert.match(app, /hero_title:\s*"在数据、变化与推断中，理解统计学"/);
  assert.match(app, /hero_title:\s*"Understand Statistics Through Data, Change, and Inference"/);
  assert.match(html, /data-i18n="clt_exp_lead"/);
  assert.match(html, /data-i18n="clt_exp_term"/);
  assert.match(html, /speakTerm\('Asymptotic'\)/);
  assert.doesNotMatch(html, /<p data-i18n="clt_exp">/);
  assert.match(app, /applyLanguage\(appState\.lang\)/);
  assert.match(app, /document\.title = currentDict\.page_title/);
  assert.match(app, /updateQuizResultsMessage\(\)/);
...
test('visible copy uses neutral learning language instead of interview framing', () => {
  ...
  assert.match(app, /bilingual explanations, and IPA\./);
  assert.match(app, /自带双语切换、音标标记。/);
});
